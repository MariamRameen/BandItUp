...
const PART_INFO = [
  { part: 1, label: "Part 1", desc: "Everyday conversation", icon: "🗣️", example: "Phone enquiry, booking, registration" },
  { part: 2, label: "Part 2", desc: "Everyday monologue",    icon: "📢", example: "Tour guide, local facility speech" },
  { part: 3, label: "Part 3", desc: "Academic discussion",   icon: "🎓", example: "University tutorial, seminar" },
  { part: 4, label: "Part 4", desc: "Academic lecture",      icon: "📚", example: "University lecture, formal talk" },
];
  ...
  return (
    <div className="min-h-screen bg-[#F7F5FF]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 py-6">
        ...
        {/* Title */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-[#1a1a2e]">Listening Practice</h1>
          <p className="text-sm text-gray-500 mt-1">Adaptive IELTS listening — select a part and accent to begin</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
                ...
                return (
                  <button key={part} onClick={() => setSelectedPart(isSelected ? null : part)}
                    className={`text-left rounded-2xl border-2 p-4 transition-all ${
                      isSelected ? "border-[#7D3CFF] bg-[#F0E8FF] shadow-md scale-[1.02]"
                                 : "border-[#F0E8FF] bg-white hover:border-[#7D3CFF] hover:shadow-sm"
                    }`}>
                    ...
                    <p className="text-xs text-gray-400 italic mb-3">{example}</p>
            ...
            <div className="bg-white rounded-2xl border border-[#F0E8FF] p-5">
              <p className="text-sm font-semibold text-[#1a1a2e] mb-3">Select Accent</p>
                ...
                {(["american", "australian"] as const).map((a) => (
                  <button key={a} onClick={() => setAccent(a)}
                    className={`flex-1 py-2.5 rounded-xl border-2 text-sm font-medium transition-all capitalize ${
                      accent === a ? "border-[#7D3CFF] bg-[#7D3CFF] text-white" : "border-[#F0E8FF] text-gray-600 hover:border-[#7D3CFF]"
                    }`}>
                    {a === "american" ? "🇺🇸 American" : "🇦🇺 Australian"}
                  </button>
                ))}
              </div>

              {error && <p className="text-sm text-red-500 mb-3 bg-red-50 rounded-xl px-4 py-2">⚠️ {error}</p>}
              ...
              <button onClick={handleStartPractice} disabled={!selectedPart || generating}
                ...
                {generating ? (
                  ...
                  <>🎧 Start Practice {selectedPart ? `— Part ${selectedPart}` : ""}</>
                )}
              </button>

              {!selectedPart && <p className="text-xs text-gray-400 text-center mt-2">Select a part above to continue</p>}
            </div>

            {/* Mock test CTA */}
            {progress?.mockAvailable && (
              <div className="bg-gradient-to-r from-[#7D3CFF] to-[#F107A3] rounded-2xl p-5 text-white">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-bold text-lg mb-1">📋 Weekly Mock Test Ready</p>
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              <div className="bg-white rounded-2xl border border-[#F0E8FF] p-8 flex justify-center">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#7D3CFF]" />
              </div>
            ) : (
              <>
                {/* Overall stats */}
                <div className="bg-white rounded-2xl border border-[#F0E8FF] p-5">
                  <p className="text-sm font-semibold text-[#1a1a2e] mb-4">Progress Overview</p>
                  <div className="text-center mb-4">
                    <p className="text-5xl font-bold text-[#7D3CFF]">{progress?.overallBand?.toFixed(1) || "—"}</p>
                    <p className="text-xs text-gray-500 mt-1">Average Band</p>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      <div key={s.label} className="bg-[#F8F9FF] rounded-xl p-3 text-center">
                        <p className="text-base font-bold text-[#1a1a2e]">{s.value}</p>
                ...
                {(progress?.weaknesses?.length ?? 0) > 0 && (
                  <div className="bg-white rounded-2xl border border-[#F0E8FF] p-5">
                    <p className="text-sm font-semibold text-[#1a1a2e] mb-3">⚠️ Focus Areas</p>
                    <div className="flex flex-wrap gap-2">
                      {progress!.weaknesses.slice(0, 5).map(w => (
                        <span key={w.type} className="text-xs bg-orange-50 text-orange-700 border border-orange-200 px-3 py-1 rounded-full">
                          {w.label}
                        </span>
                      ))}
                    </div>
                ...
                {progress?.nextMockDue && !progress.mockAvailable && (
                  <div className="bg-white rounded-2xl border border-[#F0E8FF] p-4 text-center">
                    <p className="text-xs text-gray-500 mb-1">Next mock test</p>
                ...
                <div className="flex gap-2">
                  <button onClick={() => navigate("/listening/progress")}
                    className="flex-1 border-2 border-[#F0E8FF] text-gray-600 py-2.5 rounded-xl text-sm font-medium hover:border-[#7D3CFF] hover:text-[#7D3CFF] transition-colors">
                    📊 Full Progress
                  </button>
